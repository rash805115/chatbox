...
require_once (__DIR__ . "/../config/appinfo.php");
require_once (__DIR__ . "/../config/appconstants.php");
require_once (__DIR__ . "/../model/authorization/general/authorization.php");
...
class Autoloader 
{
	...
	static $PhpsecArray = array(
			...
			"Encryption"			=>	"crypto/confidentialstring",
			"Crypt"				=>	"crypto/crypt",
			...
			"UserMessageQueue"		=>	"logs/usermessage",
			"UserMessageFormat"		=>	"logs/usermessage",
			"GenericMail"			=>	"mail/mail",
	);
	
	static $ChatboxArray = array(
			"Chat"				=>	"chat/chat",
	...
	static $FrameworkArray = array(
			...
			"DefaultViewController"		=>	"base/control",
	...
	static function autoload($ClassnameWithNamespace)
	{
		...
		elseif(substr($ClassnameWithNamespace, 0, 8) == "chatbox\\") //chatbox modules
		{
			$Classname = substr($ClassnameWithNamespace, 8);
			
			if (isset(self::$ChatboxArray[$Classname])) //chatbox modules
			{
				require_once(self::phpsecPath() . "/" . self::$ChatboxArray[$Classname] . ".php");
				return true;
			}
		}
